# 도착 시간창이 있는 컴파운드 트럭 크로스도킹 스케줄링<br>— 병목 유도 반복 지역탐색과 정확해 기반 검증 —
...
은 어떤 계산 예산에서도 실용적 이득을 주지 못하였다. 이로써 성능이
...
Joo와 Kim [4]이 도입하고 Shahmardan과 Sajadieh [5]가 발전시킨 **컴파운드
트럭(compound truck)** 변형에서는 하나의 트럭이 입고·출고 두 역할을 겸한다.
컴파운드 트럭은 여러 목적지행 제품을 싣고 도착하여 *부분적으로* 하역되며 —
...
기존 컴파운드 트럭 모델의 두 가정이 현실성을 제약한다.
...
1. **문제 확장.** 트럭별 도착 시각과 소프트 마감이 있는 컴파운드 트럭
   크로스도킹 스케줄링 문제를 — 저자가 아는 한 최초로 — 정식화한다. big-M
...
연구되었으나, 컴파운드 트럭 설정에서는 다루어진 바 없다.

### 2.2 컴파운드 트럭과 부분하역

Joo와 Kim [4]이 전용 도어 서비스 하의 컴파운드 트럭을 도입하였다.
...
본 문제의 유연성은 트럭 수준(컴파운드 트럭, 도어 간 직접 환적)에 있다. 두
...
이러한 보고는 종종 약한 베이스라인이나 동일 예산·동일 구조 통제가 없는
비교로 평가된다. 본 논문은 통제된 반례와 함께, 그러한 요소가 효과를 낼 수
없는 조건 분석을 추가한다.

### 2.5 본 연구의 위치

본 연구는 (i) 컴파운드 트럭 문제에 도착 시간창·마감을 최초로 도입하고,
(ii) 정확해로 검증되는 결정론적 근최적 휴리스틱을 제시하며, (iii) 원 문헌이
성능의 원천으로 주장한 강화학습을, 동일 예산·동일 구조 통제 하에서 부정적
...
$I$를 컴파운드 트럭, $F$를 아웃바운드 트럭, $D$를 목적지($|I| + |F| = |D|$),
$K$를 상품 종류, $M$을 도크 도어($|I| \le |M|$)의 집합이라 하자. 컴파운드
트럭 $i$는 목적지 $d$행 상품 $k$를 $f_{idk}$ 단위 싣고 있으며, 단위 핸들링
시간은 $t_k$, 도어 $m$에서 $n$으로의 배치 이송 시간은 $t_{mn}$이다. 트럭
$i$의 진입·출차 변경시간은 $DE_i$, $DL_i$이다. 핸들링 시간을
$h_{id} = \sum_k f_{idk} \cdot t_k$로 정의한다.

**결정 변수(구조).** (i) 각 컴파운드 트럭에 *유지* 목적지 하나와 도어 하나를
배정한다(도어당 컴파운드 최대 1대). (ii) 각 아웃바운드 트럭에 목적지 하나와
도어 하나를 배정한다. (iii) 같은 도어를 공유하는 아웃바운드 트럭들을
순서화한다. 각 목적지는 정확히 한 트럭(그 목적지의 *캐리어*)이 담당한다.
컴파운드 트럭은 유지 목적지 외의 수요를 모두 하역하고, 이송이 제품을 캐리어
도어로 옮기며, 각 캐리어는 전 컴파운드 트럭으로부터 모인 자기 목적지 수요를
적재한다.

**타이밍 규칙.** 평가 규칙은 [5]를 따른다. 컴파운드 트럭 $i$(유지 목적지
$d$)의 하역은 $r_i + DE_i + \sum_{d' \ne d} h_{id'}$에 끝난다. 목적지는
마지막 기여 이송이 캐리어 도어에 도착할 때 준비된다. 적재는 컴파운드
캐리어의 경우 $\max(\text{자기 하역 완료}, \text{목적지 준비})$, 아웃바운드
...
도착 시각은 컴파운드 트럭의 하역 시작을 늦추어 하류의 이송·적재 사슬 전체를
밀며, 어떤 목적지의 준비 시각은 그 목적지에 기여하는 *여러* 컴파운드 트럭의
도착 중 최댓값에 지배된다. 둘째, 유지 목적지 선택은 컴파운드 트럭의 자기
...
Vogel식 후회(regret) [12]로 컴파운드 트럭에 유지 목적지를 배정하고, 남은
...
아웃바운드 트럭의 임의 도어·위치 재배치, (b) 컴파운드 도어 스왑 및 빈 도어
...
[5]의 일반 이웃 무브 7종(목적지 스왑, 도어 스왑, 삽입)에 더해, 현재 병목을
...
- **Tabular Q**(SA-RL5 [5]의 정책): 무개선 구간 5-상태에 대한 Q-learning
  ...
  2, 비악화 1, 그 외 0).
- **Transfer DQN**: 27차원 규모 불변 상태(컴파운드 트럭 비율·화물 집중도·
...
L $(20, 30, 20)$, 컴파운드 트럭 비율 $2/3$. 화물 $f_{idk}$는 $[0, 20]$의 균등
...
학습하였으며, 아래의 모든 수치는 단 1회의 **test 풀** 실행이다: 셀당 인스턴스
5개 × 반복 5회(9셀: 3 규모 × 3 시간창 수준). CP-SAT는 인스턴스당 1회,
8스레드: S는 전 인스턴스 300초, M·L은 셀당 2인스턴스 600초. 시간창 셀에서
$\lambda = 1$. 이 분리 프로토콜은 하이퍼파라미터 튜닝과 최종 평가의 오염을
차단하여 결과의 일반화 가능성을 담보한다.
...
인스턴스별 최선해 대비 갭(Δbk; 전 방법·전 예산·CP-SAT 포함)을 주 품질
지표로, 정확해 기준이 있는 곳에서는 그 대비 갭을 함께 보고한다.

**표 6.1** 테스트 풀 결과(셀당 인스턴스 5 × 반복 5). Δbk = 인스턴스별 최선해
...
0.11~0.19% 이내이며, M-none에서는 최선 GILS 실행이 CP-SAT 인컴번트를 오히려
개선한다(Δbk < vs CP-SAT). **(ii)** GILS < Paper-SA-RL5 < VAA의 서열이
베이스라인이 정의된 모든 셀에서 성립한다. Wilcoxon 검정은 GILS-tabular가
VAA(+2.39% 평균, $n = 45$, $p < 0.0001$)와 Paper-SA-RL5(+1.16%, $n = 75$ 쌍,
$p < 0.0001$)를 유의하게 이김을 확인한다. **(iii)** 조합 하한은 무시간창
...
![선택 정책 예산 스윕: 예산별 최선해 대비 갭.](figures/fig1_selector_budget.svg)

**그림 6.3** 선택 정책 예산 스윕. 50~3,000회 전 예산에서 두 학습 정책 모두
균등 랜덤을 실용적으로 의미 있게 이기지 못한다. 전이 DQN은 일관되게 열세이며
타뷸러는 최대 예산에서만 근소 우세다.
...
우세($+0.07\%$, $p < 0.0001$), 전이 DQN은 어느 예산에서도 이기지 못하며
...
있게 만들며(재최적화는 미래를 볼 수 없다), 촉박한 실시간 결정 예산은 탐색
자체를 배제하고, 훨씬 큰 인스턴스는 예산을 포화 이전으로 되돌린다. 학습 기반
선택의 성공 보고는 그러한 체제에 집중되어 있으리라 추측하며, 강한 엔진 내부
에서 균등 선택 대비 동일 예산·동일 구조 비교 — 본 연구에서 수행한 것 — 가
표준 통제가 되어야 한다고 제안한다.
...
부분하역 컴파운드 트럭 크로스도킹 스케줄링의 **도착 시간창 확장**을 정확해
...
인스턴스를 위한 더 타이트한 하한을 연구한다. (3) $\lambda$ 민감도 분석을
확장하여 makespan-정시성 상충 곡선을 정량화한다.
